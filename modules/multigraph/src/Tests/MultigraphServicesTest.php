...
class MultigraphServicesTest extends RESTTestBase {
  ...
  public function testMultigraph() {
    ...
    foreach ($multigraphs as $name => $multigraph) {
      ...
      $this->assertEqual($response_data[$name]['uri'], Url::fromUri('user-path:monitoring-multigraph/' . $multigraph->id(), array('absolute' => TRUE))->toString());
    }
    ...
    $this->assertEqual($response_data['uri'], Url::fromUri('user-path:monitoring-multigraph/' . $multigraph->id(), array('absolute' => TRUE))->toString());
  }
